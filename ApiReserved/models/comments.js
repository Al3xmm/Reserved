...
Comments.findCommentsRestaurant=function(id, callback){
    if (connection){
        var sql=("SELECT c.contenido, c.fecha, u.nombre FROM restaurantes r, comentarios c, usuarios u WHERE c.RestauranteC="+connection.escape(id)+" AND u.IdUsuario=c.UsuarioC");
        connection.query(sql,function(error,rows){
            if (error){
                throw error;
            }else{
